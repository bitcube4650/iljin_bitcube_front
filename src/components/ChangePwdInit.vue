<template>
    <div>
        <!-- 비밀번호 변경 -->
        <div class="modal fade modalStyle" id="pwInitModal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <!-- 비밀번호 저장 얼럿 -->
        <div class="modal fade modalStyle" id="pwdInitSave" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    async savePwd(){//비밀번호 저장
        ...
        if(this.checkValid()){
            try {
                ...
                if(response.data){
                    this.reset();
                    $('#pwInitModal').modal('hide');
                    $('#pwdInitSave').modal('show');
                }
