    ...
    <div class="contents">
      <div class="conTopBox">
        <ul class="dList">
          <li>
            <div>
              조회기간 입찰완료일 기준으로 소속사의 낙찰된 입찰정보와 투찰
              정보를 확인할 수 있습니다.
            </div>
          </li>
          <li>
            <div>
              참여업체수를 클릭하면 투찰 업체들의 투찰가 및 투찰 일시를 보실 수
              있습니다.
            </div>
          </li>
        </ul>
        ...
        <div class="flex align-items-center">
          ...
          <div class="flex align-items-center width280px">
            <Calendar
              ...
              classProps="datepicker inputStyle"
              :initDate="searchParams.startDate"
            ></Calendar>
            <span style="margin: 0 10px">~</span>
            <Calendar
              @update-date="fnUpdateEndDate"
              calendarId="endDate"
              classProps="datepicker inputStyle"
              :initDate="searchParams.endDate"
            ></Calendar>
          </div>
          ...
          <div class="flex align-items-center width300px" v-if="lotteMat">
            <select v-model="searchParams.matDept" class="selectStyle">
              ...
              <option
                v-for="(data1, index) in lotteMatCode.matDept"
                :key="'dept_' + index"
                :value="data1.codeVal"
              >
                {{ data1.codeName }}
            ...
            <select v-model="searchParams.matProc" class="selectStyle ml10">
              ...
              <option
                v-for="(data2, index) in lotteMatCode.matProc"
                :key="'proc_' + index"
                :value="data2.codeVal"
              >
                {{ data2.codeName }}
            ...
            <select v-model="searchParams.matCls" class="selectStyle ml10">
              ...
              <option
                v-for="(data3, index) in lotteMatCode.matCls"
                :key="'cls_' + index"
                :value="data3.codeVal"
              >
                {{ data3.codeName }}
      ...
      <div class="flex align-items-center justify-space-between mt40">
        ...
        <div class="flex-shrink0">
          <a
            ...
            title="엑셀 다운로드"
            >엑셀 다운로드 <i class="fa-light fa-arrow-down-to-line ml10"></i
          ></a>
        </div>
      </div>
        ...
        <table class="tblSkin1 mt10">
          ...
          <tbody>
            <tr v-for="(data, index) in listPage.content" :key="index">
              ...
              <td>
                <a
                  @click="$refs.joinCustList.initModal(data.biNo)"
                  data-toggle="modal"
                  data-target="#companiesNum"
                  class="textUnderline"
                  title="투찰 정보 페이지가 열림"
                  >{{ data.joinCustCnt }}</a
                >
...
export default {
  ...
  methods: {
    ...
    fnExcelDown() {
      ...
      this.$store.commit("loading");
      this.$http
        .post("/api/v1/excel/bid/completeList/downLoad", params, {
          responseType: "blob",
        })
        .then((response) => {
  ...
  mounted() {
    ...
    this.searchParams.startDate = cmmn.strDateAddDay(
